...
class TBT_Rewards_Block_Manage_Promo_Quote_Edit_Tabs extends Mage_Adminhtml_Block_Widget_Tabs {
	...
	public function __construct() {
		...
		if ($this->_isRedemptionType ()) {
			$this->setTitle ( Mage::helper ( 'rewards' )->__ ( 'Shopping Cart Points Spending Rule' ) );
		} else {
			$this->setTitle ( Mage::helper ( 'rewards' )->__ ( 'Shopping Cart Points Earning Rule' ) );
		}
	}
	
	protected function _beforeToHtml() {
		$this->addTab ( 'main_section', array ('label' => Mage::helper ( 'salesrule' )->__ ( 'Rule Information' ), 'content' => $this->getLayout ()->createBlock ( 'rewards/manage_promo_quote_edit_tab_main' )->toHtml (), 'active' => true ) );
		
		$this->addTab ( 'conditions_section', array ('label' => Mage::helper ( 'salesrule' )->__ ( 'Conditions' ), 'content' => $this->getLayout ()->createBlock ( 'rewards/manage_promo_quote_edit_tab_conditions' )->toHtml () ) );
		
		$this->addTab ( 'actions_section', array ('label' => Mage::helper ( 'salesrule' )->__ ( 'Actions' ), 'content' => $this->getLayout ()->createBlock ( 'rewards/manage_promo_quote_edit_tab_actions' )->toHtml () ) );
		
		if (Mage::helper ( 'rewards' )->isBaseMageVersionAtLeast ( '1.4.0.0' ) && $this->_isRedemptionType ()) {
