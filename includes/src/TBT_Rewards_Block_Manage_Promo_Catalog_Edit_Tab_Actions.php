...
class TBT_Rewards_Block_Manage_Promo_Catalog_Edit_Tab_Actions extends Mage_Adminhtml_Block_Widget_Form {
	...
	protected function _prepareForm() {
		...
		// Dispatch event 
		Mage::dispatchEvent( 'rewards_block_manage_actions_setup_catalogrule_before', array(
            'actions_block' => $this,
		    'catalogrule'	=> $model,
		    'form'			=> $form
        ) );
		...
		if ($this->_isDistributionType ()) {
		    ...
		    Mage::getSingleton('rewards/wikihints')->addWikiHint($fieldset, "21237356", "Catalog Earning Rule - Actions" );
		    ...
		    Mage::getSingleton('rewards/wikihints')->addWikiHint($points_max_qty_field, "21626033", "Catalog Points Earning Rule: Maximum Total of Points To Transfer" );
			...
			Mage::getSingleton('rewards/wikihints')->addWikiHint($fieldset, "21208761", "Catalog Points Spending Rule - Actions - Discount Amount" );
			...
			Mage::getSingleton('rewards/wikihints')->addWikiHint($element, "21483136", "Stop further rules processing", null, $this->__("Get help with the Stop Further Rules Processing flag."));
			...
			Mage::getSingleton('rewards/wikihints')->addWikiHint($points_uses_per_product_field, "21612941", "Catalog Points Spending Rule - Actions - Uses Allowed Per Product" );
		...
		// Dispatch event 
		Mage::dispatchEvent( 'rewards_block_manage_actions_setup_catalogrule_after', array(
            'actions_block' => $this,
		    'rule'	        => $model,
		    'form'			=> $form
        ) );
