...
class TBT_Rewards_Model_Salesrule_Validator extends Mage_SalesRule_Model_Validator {
	...
	protected function recalculateDiscounts($quote, $address, $item, $rule_id) {
		...
		$rulePercent = min ( 100, $rule->getPointsDiscountAmount () );
		$discountAmount = 0;
		...
		switch ($rule->getPointsDiscountAction ()) {
			case 'to_percent' :
			//@nelkaake -a 16/11/10: THIS TYPE OF DISCOUNT WAS ABANDONED BY MAGENTO
			//$rulePercent = max(0, 100-$rule->getPointsDiscountAmount());
			//no break;
			...
			case 'to_fixed' :
				$quoteAmount = $quote->getStore ()->convertPrice ( $rule->getPointsDiscountAmount () );
				$discountAmount = $qty * ($item->getCalculationPrice () - $quoteAmount);
				$baseDiscountAmount = $qty * ($item->getBaseCalculationPrice () - $rule->getPointsDiscountAmount ());
				break;
			...
			case 'by_fixed' :
				...
				$baseDiscountAmount = $qty * $rule->getPointsDiscountAmount ();
				break;
			...
			case 'buy_x_get_y' :
				...
				$y = $rule->getPointsDiscountAmount ();
				if (! $x || $y >= $x) {
					break;
				}
				$buy = 0;
	...
	protected function _getTotalPercentDiscount($item, $address, $rule, $qty = null) {
		...
		if ($rule->getPointsAction () != TBT_Rewards_Model_Salesrule_Actions::ACTION_DISCOUNT_BY_POINTS_SPENT) {
			$discountPercent = ($rule->getPointsDiscountAmount () / 100);
	...
	protected function _getTotalFixedDiscountOnitem($item, $address, $rule, &$cartRules) {
		...
		if (Mage::helper ( 'rewards' )->isBaseMageVersionAtLeast ( '1.4.2.0' )) {
			...
			if (1 >= $this->_rulesItemTotals [$rule->getId ()] ['items_count']) {
				...
				$maximumItemDiscount = $rule->getPointsDiscountAmount () * $discountRate;
		...
		foreach ( $this->_getRules () as $rule ) {
			...
			switch ($rule->getPointsDiscountAction ()) {
				...
				case 'to_fixed' :
					...
					$baseDiscountAmount = $baseShippingAmount - $rule->getPointsDiscountAmount ();
					break;
				case 'by_fixed' :
					$quoteAmount = $quote->getStore ()->convertPrice ( $rule->getPointsDiscountAmount () );
					$discountAmount = $quoteAmount;
					$baseDiscountAmount = $rule->getPointsDiscountAmount ();
					break;
				...
				case 'cart_fixed' :
					...
					if (! isset ( $cartRules [$rule->getId ()] )) {
						$cartRules [$rule->getId ()] = $rule->getPointsDiscountAmount ();
					}
					if ($cartRules [$rule->getId ()] > 0) {
						$quoteAmount = $quote->getStore ()->convertPrice ( $cartRules [$rule->getId ()] );
		...
		foreach ( $this->_getRules () as $rule ) {
			...
			switch ($rule->getPointsDiscountAction ()) {
				...
				case 'to_fixed' :
					...
					$baseDiscountAmount = $qty * ($baseItemPrice - $rule->getPointsDiscountAmount ());
					break;
