    ...
    implements Mana_Filters_Interface_Filter
{
	...
	public function getLowestPossibleValue() {
		return (int)$this->getMinPriceSlider();
	}
	...
	protected $_isMinPriceIntCalculated = false;
    protected $_minPriceInt;
    public function getMinPriceSlider() {
        $minPrice = null;
		if(strpos($_SERVER['REQUEST_URI'],'price')){
			return Mage::getSingleton('core/session')->getLowestPossibleValue();
		}
        if (is_null($minPrice)) {
			$SQL = $this->getLayer()->getProductCollection()->getSelect();
            //$items = $this->getLayer()->getProductCollection();
            $write = Mage::getSingleton('core/resource')->getConnection('core_write');
			$readresult=$write->query($SQL);
			$minPrices = array();
            while ($row = $readresult->fetch()){
            	$minPrices[] = $row['minimal_price'];
            }
            sort($minPrices);
            if(count($minPrices)){
            	$minPrice = $minPrices[0];
            }
			Mage::getSingleton('core/session')->setLowestPossibleValue($minPrice);
            //$this->setData('min_price_slider', $minPrice);
            return $minPrice;
        }
        return 0;
    }
