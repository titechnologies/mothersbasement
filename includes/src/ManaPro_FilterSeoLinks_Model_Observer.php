...
class ManaPro_FilterSeoLinks_Model_Observer extends Mage_Core_Helper_Abstract {
    ...
    public function addAppliedFiltersToTitle($observer) {
        ...
        if ($head = $layout->getBlock('head')) {
            ...
            if ($core->getRoutePath() == 'catalog/category/view') {
                if ($appliedFilters = $layer->getState()->getFilters()) {
                    $core->callProtectedMethod(array($action, '_renderTitles'));
                    $this->_addAppliedFiltersToCategoryTitle($head, $appliedFilters);
                }
            } elseif ($core->getRoutePath() == 'catalogsearch/result/index') {
                if ($appliedFilters = $layer->getState()->getFilters()) {
                    $core->callProtectedMethod(array($action, '_renderTitles'));
                    $this->_addAppliedFiltersToSearchTitle($head, $appliedFilters);
                }
            }
        }
    }
    protected function _addAppliedFiltersToCategoryTitle($head, $appliedFilters) {
    ...
    protected function _addAppliedFiltersToSearchTitle($head, $appliedFilters) {
        if (count($appliedFilters)) {
            $title = array();
            ...
            if ($title = implode(', ', $title)) {
                $head->setTitle($this->_getInitialTitle($head) . ': ' . $title);
            }
        }
    }
