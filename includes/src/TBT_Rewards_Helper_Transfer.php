...
class TBT_Rewards_Helper_Transfer extends Mage_Core_Helper_Abstract {
    ...
    {
        $customerId = Mage::getModel('sales/order')->load($order_id)->getCustomerId();
            ...
            foreach ( $item_rule_ids as $item_rule_id ) {
                ...
                if ($item_rule->getId () == $required_id) {
                    if (Mage::helper ( 'rewards/config' )->calcCartPointsAfterDiscount ()) {
                        ...
                        if ($prices_include_tax) {
                            // we don't fall back to getBaseRowTotal here because applied redemptions can legitimately drop this value to 0
        ...
        foreach ( $order_items as $item ) {
            ...
            foreach ( $item_rule_ids as $item_rule_id ) {
                if ($item_rule_id == $required_id) {
                    ...
                    $discount += Mage::helper('rewards/price')->getReversedCurrencyPrice($item->getDiscountAmount());
