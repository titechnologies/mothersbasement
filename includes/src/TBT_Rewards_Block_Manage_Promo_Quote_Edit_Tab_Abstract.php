	...
	protected function _getPriceActionFieldset(Varien_Data_Form &$form, TBT_Rewards_Model_Salesrule_Rule &$model) {
		...
		*/
		if ($this->_isRedemptionType ()) {
		    ...
		    Mage::getSingleton('rewards/wikihints')->addWikiHint($fieldset, "21243116", "Shopping Cart Spending Rule - Actions - Update prices using the following information" );
			
			$fieldset->addField ( 'points_discount_action', 'select', array ('label' => $apply_caption, 'name' => 'points_discount_action', 'options' => $actions, 'onchange' => 'toggleDiscountActionsSelect(this.value)' ) );
			
			$fieldset->addField ( 'points_discount_amount', 'text', array ('name' => 'points_discount_amount', 'required' => true, 'class' => 'validate-not-negative-number', 'label' => Mage::helper ( 'rewards' )->__ ( 'Discount Amount' ) ) );
...
*/
			...
			//@nelkaake -a 16/11/10: 
			if ($this->_isRedemptionType () && Mage::helper ( 'rewards' )->isMageVersionAtLeast ( '1.4.0.0' )) {
		    ...
		    Mage::getSingleton('rewards/wikihints')->addWikiHint($stop_rules_processing_field, "21483136", "Shopping Cart Spending Rule - Actions - Stop Further Rules Processing" );
		...
		$model->setPointsDiscountAmount ( $model->getPointsDiscountAmount () * 1 );
		return $this;
	...
	protected function _getApplyToActionFieldset(&$form, &$model) {
		...
		Mage::getSingleton('rewards/wikihints')->addWikiHint($fieldset, "21237846", "How to apply the rule actions to a subset of products" );
	...
	protected function _getPointsActionFieldset(&$form) {
		...
		if ($this->_isRedemptionType ()) {
		    Mage::getSingleton('rewards/wikihints')->addWikiHint($fieldset, "21243116", "Shopping Cart Spending Rule - Conditions" );
		} else {
		    Mage::getSingleton('rewards/wikihints')->addWikiHint($fieldset, "21218462", "Shopping Cart Earning Rule - Actions" );
		}
		...
		if ($this->_isRedemptionType ()) {
		    Mage::getSingleton('rewards/wikihints')->addWikiHint($points_max_qty_field, "21618532", "Shopping Cart Points Spending Rule: Maximum Distributed Points" );
		} else {
		    Mage::getSingleton('rewards/wikihints')->addWikiHint($points_max_qty_field, "21618532", "Shopping Cart Points Earning Rule: Maximum Distributed Points" );
		}
