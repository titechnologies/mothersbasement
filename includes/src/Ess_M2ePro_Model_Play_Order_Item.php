...
class Ess_M2ePro_Model_Play_Order_Item extends Ess_M2ePro_Model_Component_Child_Play_Abstract
{
    ...
    /** @var $playItem Ess_M2ePro_Model_Play_Item */
    private $playItem = NULL;
    ...
    {
        if (is_null($this->playItem)) {
            $this->playItem = Mage::getModel('M2ePro/Play_Item')->getCollection()
        ...
        return !is_null($this->playItem->getId()) ? $this->playItem : NULL;
    }
    ...
    public function getAssociatedProductId()
    {
        ...
        // ----------------
        if (!is_null($this->getPlayItem())) {
            return $this->getPlayItem()->getProductId();
        }
        // ----------------
    ...
    {
        $playItem = $this->getPlayItem();

        if (!is_null($playItem) && !$this->getPlayAccount()->isMagentoOrdersListingsModeEnabled()) {
            throw new Exception(
                'Magento Order creation for items listed by M2E Pro is disabled in Account settings.'
            );
        }

        if (is_null($playItem) && !$this->getPlayAccount()->isMagentoOrdersListingsOtherModeEnabled()) {
            throw new Exception(
