...
/**
 * Used to render the Developer/SecretKey field on the Config page using a non-default config path
 * @category   TBT
 * @package    TBT_Rewards
 * @author     Sweet Tooth Team <contact@sweettoothhq.com>
 */
class TBT_Rewards_Block_System_Config_Developer_SecretKey extends Mage_Adminhtml_Block_System_Config_Form_Field
{
    /**
     * Renders the element into HTML, using the Secret Key as the value to render
     * @param Varien_Data_Form_Element_Abstract $element The element to render
     * @return string
     */
    public function render(Varien_Data_Form_Element_Abstract $element)
    {
        $secretKey = Mage::helper('core')->decrypt(Mage::getStoreConfig('rewards/platform/secretkey'));
        $element->setValue($secretKey);
