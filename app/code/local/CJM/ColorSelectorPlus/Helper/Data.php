		...
		if($product_base):
			...
			foreach($images as $key => $image):
				if($product_base['defaultimg'][$key] == 1 && $product_base['morev'][$key] != ''):
					...
					$onloads .= '<li class="onload-moreviews"><a href="'.$product_base['full'][$key].'" rel="popupWin:\''.$product_gallery.'\', useZoom: \'cloudZoom\', smallImage: \''.$product_image.'\'" class="cloud-zoom-gallery" title="'.$product_label.'"><img src="'.$product_thumb.'" width="'.$theSizes['more']['width'].'" height="'.$theSizes['more']['height'].'" alt="'.$product_label.'" title="'.$product_label.'" /></a></li>';	
				elseif($product_base['morev'][$key] == ''):
					$defaultmoreviews = 1;
					$product_image = $product_base['image'][$key];
					$product_thumb = $product_base['thumb'][$key];
					$product_label = $product_base['label'][$key];
					$product_gallery = Mage::helper('colorselectorplus')->getGalUrl($product_base['id'][$key], $_product->getId());
					$defaults .= '<li class="default-moreviews"><a href="'.$product_base['full'][$key].'" rel="popupWin:\''.$product_gallery.'\', useZoom: \'cloudZoom\', smallImage: \''.$product_image.'\'" class="cloud-zoom-gallery" title="'.$product_label.'"><img src="'.$product_thumb.'" width="'.$theSizes['more']['width'].'" height="'.$theSizes['more']['height'].'" alt="'.$product_label.'" title="'.$product_label.'" /></a></li>';
				endif;
			endforeach;
					...
					foreach($options as $option):
						...
						foreach($images as $key => $image):
							if($product_base['morev'][$key] == $value):
								...
								$product_label = $product_base['label'][$key];
								$product_gallery = Mage::helper('colorselectorplus')->getGalUrl($product_base['id'][$key], $_product->getId());
								$html .= '<li class="moreview'.$value.'" style="display:none;"><a href="'.$product_base['full'][$key].'" rel="popupWin:\''.$product_gallery.'\', useZoom: \'cloudZoom\', smallImage: \''.$product_image.'\'" class="cloud-zoom-gallery" title="'.$product_label.'"><img src="'.$product_thumb.'" width="'.$theSizes['more']['width'].'" height="'.$theSizes['more']['height'].'" alt="'.$product_label.'" title="'.$product_label.'" /></a></li>';
							endif;
						endforeach;
               		endforeach;
				endif;
