...
/*
            elseif(($haveregular==0)&&(($orderStatus=='processing')) && ($invoices_created > 0) && ($invoices_created < $totalitems))
            {
...
*/
        ...
        {
            $_items         =   $order->getItemsCollection();
            $totalitems         =   sizeof($order->getItemsCollection());	//NEW - 27-09-2013
            $haveregular    =   Mage::helper('aitpreorder')->isHaveReg($_items,1);
            ...
            if(($haveregular==0)&&($orderStatus=='pending'))
            {
                $tmpstatus          =   'pendingpreorder';
                $this->setStatusPreorder('pendingpreorder');
            //ORIGINAL } else if($isHavePreorder && $orderStatus=='pending' && ($invoices_created > 0) ) {
	    } else if(($isHavePreorder == 0) && $orderStatus=='pending' && ($invoices_created > 0) && ($invoices_created < ($totalitems-1)) ) {

                $tmpstatus          =   'pendingpreorder';
                $this->setStatusPreorder('pendingpreorder');

            }elseif(($haveregular==0) && ($orderStatus=='pendingpreorder') && ($invoices_created > 0) && ($invoices_created < $totalitems)){
                $tmpstatus      =   'processingpreorder';
                $this->setStatusPreorder('processingpreorder');
            }elseif(($haveregular!=0) && ($isHavePreorder!=0) && ($orderStatus=='pendingpreorder') && ($invoices_created > 0) && ($invoices_created < $totalitems)){
                $tmpstatus      =   'pendingpreorder';
                $this->setStatusPreorder('pendingpreorder');

            }elseif(($haveregular==1) && ($orderStatus=='pendingpreorder') && ($invoices_created > 0) && ($invoices_created < $totalitems)){
                $tmpstatus      =   'processingpreorder';
                $this->setStatusPreorder('processingpreorder');
