...
class Aitoc_Aitsys_Model_Aitfilesystem extends Aitoc_Aitsys_Abstract_Model
{
    ...
    protected $_forbiddenDirs = array('.', '..', '.svn');
    ...
    public function grantAll( $path , $recursive = true, $all = false )
    {
        if(file_exists($path))
        {
            ...
            if ($recursive = is_dir($path))
            {
                ...
                {
                    if(in_array(basename($item), $this->_forbiddenDirs))
                    {
                        continue;
                    }
    ...
    public function checkMainPermissions()
    {
        if (is_null($this->_closedMainFolders))
        {
            ...
            $modulesDirs    = $this->getAitocModulesDirs();
            
            $foldersToCheck[] = dirname(Mage::getRoot()) . DIRECTORY_SEPARATOR . 'var' . DIRECTORY_SEPARATOR;
    ...
    public function getSubsystemsDirs()
    {
        if(empty($this->_subsystemsDirs))
        {
            ...
            $this->_subsystemsDirs[] = $root . Aitoc_Aitsys_Model_Rewriter_Abstract::REWRITE_CACHE_DIR;
            $this->_subsystemsDirs[] = $root . DIRECTORY_SEPARATOR . 'var' . DIRECTORY_SEPARATOR . Aitoc_Aitsys_Model_Platform::INSTALLATION_DIR . DIRECTORY_SEPARATOR;
            $this->_subsystemsDirs[] = $root . DIRECTORY_SEPARATOR . 'var' . DIRECTORY_SEPARATOR . Aitoc_Aitsys_Model_Aitpatch::PATCH_DIR . DIRECTORY_SEPARATOR;
        }
	...
	public function getSubDir($directoryPath)
	{
		...
		{
			if (!in_array($file, $this->_forbiddenDirs) && is_dir($directoryPath.$file))
			{
				$directories[] = $file;
			}
		}
