...
class Mage_Catalog_Block_Product_List_Toolbar extends Mage_Core_Block_Template
{
    ...
    protected function _construct()
    {
        ...
        $this->addOrderToAvailableOrders("created_at","Date Added");
...
//     	if($order == 'popularity') {
// 	    	$popular = Mage::getResourceModel('reports/product_collection')
// 				    	->addAttributeToSelect('*')
// 				    	->setStoreId($storeId)
// 				    	->addStoreFilter($storeId)
// 				    	->addViewsCount();
// 	    	//$collection = array_merge( $collection, $select );
// 	    	//$collection->getSelect()->union(array($popular->getSelect()));
// 	    	$collection = $popular;
//     	} 
...
//     }
    public function setCollection($collection)
    {
    	...
    	if($this->getCurrentOrder() == 'popularity'){
        	...
        	$this->_collection->getSelect()->join(array("productreport"=>"report_viewed_product_index"),"productreport.product_id=e.entity_id",array('productcount'=>"COUNT(productreport.product_id)"))->group("e.entity_id")->order("productcount ".$this->getCurrentDirection());
        }
