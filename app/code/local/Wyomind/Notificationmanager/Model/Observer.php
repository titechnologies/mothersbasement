...
class Wyomind_Notificationmanager_Item {
    ...
    public function Wyomind_Notificationmanager_Item() { $this->__contruct(); }
    
    public function setPubDate($date)     { $this->pubDate = $date; }
    public function setTitle($title)      { $this->title = $title; }
    public function setDescription($desc) { $this->description = $desc; }
    public function setSeverity($sev)     { $this->severity = $sev; }
			...
			if ($rss != NULL) {
				...
				if ($items) {
					foreach ($items as $item) {
						...
						if ($infos->identifier == "Global" || in_array($infos->identifier,$exts)) {
