...
class FME_Layaway_Model_Quote_Admintotal2 extends Mage_Sales_Model_Quote_Address_Total_Abstract {
		...
		$quote = $address->getQuote();
			...
			if($item->getIsLayaway()){
				...
				if(!$item->getParentItemId()){
					...
					$layawayremaining += ($item->getBasePrice() - ($item->getBasePrice() * Mage::helper('layaway')->getLayawayFee('layaway_first',$data)/100))* $item->getQty();
				    }else{
					 $layawayremaining += ($item->getPrice() - ($item->getPrice() * Mage::helper('layaway')->getLayawayFee('layaway_first',$data)/100))* $item->getQty();
				    }
...
//                        $address->setBaseGrandTotal($address->getBaseGrandTotal() - $address->getLayawayRemaining());
