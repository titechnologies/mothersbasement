...
class Ess_M2ePro_Adminhtml_Play_ListingController extends Ess_M2ePro_Controller_Adminhtml_MainController
{
    //#############################################
    ...
    protected function _initAction()
    {
        ...
        $this->getLayout()->getBlock('head')
            ...
            ->addJs('M2ePro/Play/Listing/SettingsHandler.js')
            ->addJs('M2ePro/Play/Listing/ChannelSettingsHandler.js')
            ->addJs('M2ePro/Play/Listing/CategoryHandler.js')
            ->addJs('M2ePro/Listing/MoveToListingHandler.js')
            ->addJs('M2ePro/Play/Listing/ProductSearchHandler.js')
    ...
    {
        $block = $this->loadLayout()->getLayout()->createBlock('M2ePro/adminhtml_play_listing_grid');
    ...
    {
        $this->_initAction()
            ->_addContent($this->getLayout()->createBlock('M2ePro/adminhtml_play_listing_search'))
            ->renderLayout();
    }

    public function searchGridAction()
    {
        $block = $this->loadLayout()->getLayout()->createBlock('M2ePro/adminhtml_play_listing_search_grid');
        $this->getResponse()->setBody($block->toHtml());
    }
    ...
    public function createListingAction()
    {
        ...
        // Add new listing
        //---------------
        $generalTemplate = Mage::helper('M2ePro/Component_Play')->getModel('Template_General');
        $generalTemplate->addData($sessionData)->save();

        $descriptionTemplatesCollection = Mage::helper('M2ePro/Component_Play')->getCollection('Template_Description');
        if ($descriptionTemplatesCollection->getSize() > 0) {
            $descriptionTemplate = $descriptionTemplatesCollection->getFirstItem();
        } else {
            $descriptionTemplate = Mage::helper('M2ePro/Component_Play')->getModel('Template_Description');
        ...
        if ($isEmptyListing == 1) {
            if ($this->getRequest()->getParam('back') == 'list') {
                $backUrl = $this->getUrl('*/adminhtml_listing/index', array('tab' => 'play'));
            } else {
    ...
    public function addStepOne()
    {
        ...
        $this->_initAction()
            ->_addContent($this->getLayout()->createBlock('M2ePro/adminhtml_play_listing_add_stepOne'))
            ->renderLayout();
    }
    ...
    public function addStepTwo()
    {
        ...
        $this->_initAction()
            ->_addContent($this->getLayout()->createBlock('M2ePro/adminhtml_play_listing_add_stepTwo'))
            ->renderLayout();
    }
    ...
    public function addStepThree()
    {
        ...
        $this->_initAction()
            ->_addContent($this->getLayout()->createBlock('M2ePro/adminhtml_play_listing_add_stepThree'))
            ->renderLayout();
    }
    ...
    public function addStepFour()
    {
        ...
        if ($temp['source_products'] == Ess_M2ePro_Model_Listing::SOURCE_PRODUCTS_CUSTOM) {
            $blockContent = $this->getLayout()->createBlock('M2ePro/adminhtml_play_listing_add_stepFourProduct');
        } else if ($temp['source_products'] == Ess_M2ePro_Model_Listing::SOURCE_PRODUCTS_CATEGORIES) {
            $blockContent = $this->getLayout()->createBlock('M2ePro/adminhtml_play_listing_add_stepFourCategory');
        } else {
            $this->_redirect('*/*/add',array('step'=>'1','clear'=>'yes'));
            return;
        }
    ...
    public function addStepFive()
    {
        ...
        $this->_initAction()
            ->_addContent($this->getLayout()->createBlock('M2ePro/adminhtml_play_listing_add_StepFive'))
            ->renderLayout();
    }
    ...
    public function viewAction()
    {
        ...
        $this->_initAction()
            ->_addContent($this->getLayout()->createBlock('M2ePro/adminhtml_play_listing_view'))
            ->renderLayout();
    }
    ...
    public function viewGridAction()
    {
        ...
        $response = $this->loadLayout()->getLayout()
            ->createBlock('M2ePro/adminhtml_play_listing_view_grid')->toHtml();
    ...
    public function editAction()
    {
        ...
        $additionalData = array(
            ...
            'dispatch_to_mode' => $generalTemplate->getDispatchToMode(),
            'dispatch_to_value' => $generalTemplate->getData('dispatch_to_value'),
            'dispatch_to_custom_attribute' => $generalTemplate->getData('dispatch_to_custom_attribute'),

            'dispatch_from_mode' => $generalTemplate->getDispatchFromMode(),
            'dispatch_from_value' => $generalTemplate->getData('dispatch_from_value'),

            'shipping_price_gbr_mode' => $generalTemplate->getShippingPriceGbrMode(),
            'shipping_price_gbr_value' => $generalTemplate->getData('shipping_price_gbr_value'),
            'shipping_price_gbr_custom_attribute' => $generalTemplate->getData('shipping_price_gbr_custom_attribute'),

            'shipping_price_euro_mode' => $generalTemplate->getShippingPriceEuroMode(),
            'shipping_price_euro_value' => $generalTemplate->getData('shipping_price_euro_value'),
            'shipping_price_euro_custom_attribute' => $generalTemplate->getData('shipping_price_euro_custom_attribute')
        );

        Mage::helper('M2ePro')->setGlobalValue('temp_data', array_merge($model->getData(), $additionalData));

        $this->_initAction();
        $this->_addLeft($this->getLayout()->createBlock('M2ePro/adminhtml_play_listing_edit_tabs'));
        $this->_addContent($this->getLayout()->createBlock('M2ePro/adminhtml_play_listing_edit'));
        $this->renderLayout();
    }
    ...
    public function deleteAction()
    {
        ...
        foreach ($idsForDelete as $id) {
            $template = Mage::helper('M2ePro/Component_Play')->getCachedObject('Listing',$id);
    ...
    public function productAction()
    {
        ...
        if ($this->getRequest()->isPost()) {
            ...
            if ($this->getRequest()->getParam('do_list')) {
                $redirectUrl = $this->getUrl('*/adminhtml_play_listing/view', array('id'=>$id));
        ...
        $this->_initAction()
            ->_addContent($this->getLayout()->createBlock('M2ePro/adminhtml_play_listing_product'))
            ->renderLayout();
    }
    ...
    public function categoryProductAction()
    {
        ...
        if (!is_null($addProducts)) {
            ...
            $this->_initAction();
            $this->_addContent($this->getLayout()->createBlock('M2ePro/adminhtml_play_listing_product'));
            $this->renderLayout();
        ...
        $blockContent = $this->getLayout()->createBlock(
            'M2ePro/adminhtml_play_listing_product_category'
        );
    ...
    public function productGridAction()
    {
        ...
        $response = $this->loadLayout()->getLayout()
            ->createBlock('M2ePro/adminhtml_play_listing_product_grid')->toHtml();
        $this->getResponse()->setBody($response);
    }
    ...
    public function tryToMoveToListingAction()
    {
        ...
        foreach ($selectedProducts as $selectedProduct) {
            $listingProductInstance = Mage::helper('M2ePro/Component_Play')
    ...
    protected function processConnector($action, array $params = array())
    {
        ...
        if ($result == Ess_M2ePro_Model_Play_Connector_Product_Requester::STATUS_ERROR) {
            return json_encode(
                array('result'=>'error','action_id'=>$actionId,'is_processing_items'=>$isProcessingItems)
            );
        }

        if ($result == Ess_M2ePro_Model_Play_Connector_Product_Requester::STATUS_WARNING) {
            return json_encode(
                array('result'=>'warning','action_id'=>$actionId,'is_processing_items'=>$isProcessingItems)
            );
        }

        if ($result == Ess_M2ePro_Model_Play_Connector_Product_Requester::STATUS_SUCCESS) {
            return json_encode(
    ...
    {
        exit($this->processConnector(Ess_M2ePro_Model_Play_Connector_Product_Dispatcher::ACTION_LIST));
    }

    public function runReviseProductsAction()
    {
        exit($this->processConnector(Ess_M2ePro_Model_Play_Connector_Product_Dispatcher::ACTION_REVISE));
    }

    public function runRelistProductsAction()
    {
        exit($this->processConnector(Ess_M2ePro_Model_Play_Connector_Product_Dispatcher::ACTION_RELIST));
    }

    public function runStopProductsAction()
    {
        exit($this->processConnector(Ess_M2ePro_Model_Play_Connector_Product_Dispatcher::ACTION_STOP));
    }

    public function runStopAndRemoveProductsAction()
    {
        exit($this->processConnector(
            Ess_M2ePro_Model_Play_Connector_Product_Dispatcher::ACTION_STOP, array('remove' => true)
        ));
    }
    ...
    public function getSuggestedPlayIDGridAction()
    {
        ...
        if (!empty($suggestedData)) {
            ...
            $response = $this->loadLayout()->getLayout()
                ->createBlock('M2ePro/adminhtml_play_listing_productSearch_grid')->toHtml();
        } else {
    ...
    public function mapToPlayIDAction()
    {
        ...
        if ($listingProduct->isNotListed() &&
            ...
            $listingProduct->setData(
                'general_id_type', Ess_M2ePro_Model_Play_Template_General::GENERAL_ID_MODE_GENERAL_ID
            );
    ...
    public function unmapFromPlayIDAction()
    {
        ...
        foreach ($productIds as $productId) {
            ...
            $listingProduct->setData(
                'general_id_type',Ess_M2ePro_Model_Play_Template_General::GENERAL_ID_MODE_NOT_SET
            );
            $listingProduct->setData('category_id',NULL);
            $listingProduct->setData('general_id_search_status',$temp);
            $listingProduct->setData('general_id_search_suggest_data',NULL);
    ...
    {
        Mage::helper('M2ePro/Module')->getConfig()->setGroupValue('/cache/play/listing/', 'tutorial_shown',1);
        $this->_redirect('*/adminhtml_listing/index',
