...
class Ess_M2ePro_Block_Adminhtml_Amazon_Listing_View_Grid extends Ess_M2ePro_Block_Adminhtml_Listing_View_Grid
{
...
//                                '(csi.product_id = `main_table`.product_id)',array('qty'))
    ...
    protected function _prepareMassaction()
    {
        ...
        $this->getMassactionBlock()->addItem('new_general_id', array(
    ...
    private function getGeneralIdColumnValueEmptyGeneralId($row)
    {
        ...
        if (!empty($templateNewProductId)) {
            ...
            $newAsinTemplateUrl = $this->getUrl(
                '*/adminhtml_amazon_template_newProduct/edit/',
                array(
                    'id' => $templateNewProductId,
                    'marketplace_id' => $marketplaceId,
                    'listing_product_id' => $lpId,
                    'save_and_assign' => 0,
                    'back' => Mage::helper('M2ePro')->makeBackUrlParam('*/adminhtml_amazon_listing/view',array(
...
<a href="javascript: void(0);" title="{$tip}"
   onclick="AmazonListingProductSearchHandlerObj.showUnmapFromGeneralIdPrompt('{$lpId}');">
...
{$na} &nbsp;
<a href="javascript: void(0);" title="{$tip}"
    onclick="AmazonListingProductSearchHandlerObj.openPopUp(0,'{$productTitle}',{$lpId},'{$tip}');">
    <img src="{$iconSrc}" alt="" width="16" height="16">
</a>
HTML;
        }
...
<a href="javascript:;" title="{$tip}"
   onclick="AmazonListingProductSearchHandlerObj.openPopUp(1,'{$productTitle}',{$lpId})">
    <img src="{$iconSrc}" alt="" width="16" height="16">
</a>
HTML;
        }
...
{$na} &nbsp;
<a href="javascript:;" title="{$tip}"
   onclick="AmazonListingProductSearchHandlerObj.openPopUp(0,'{$productTitle}',{$lpId});">
    <img src="{$iconSrc}" alt="" width="16" height="16">
</a>
HTML;
    }
