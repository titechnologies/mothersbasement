...
class Ess_M2ePro_Model_Play_Order extends Ess_M2ePro_Model_Component_Child_Play_Abstract
{
    ...
    const SHIPPING_STATUS_INCOMPLETE = 0;
    const SHIPPING_STATUS_COMPLETED  = 1;

    // ########################################

    private $relatedPlayItems = NULL;
    ...
    {
        if (is_null($this->relatedPlayItems)) {
            $this->relatedPlayItems = array();

            foreach ($this->getParentObject()->getItemsCollection()->getItems() as $item) {
                if (!is_null($item->getChildObject()->getPlayItem())) {
                    $this->relatedPlayItems[] = $item->getChildObject()->getPlayItem();
                }
            }
        }

        return $this->relatedPlayItems;
    }
    ...
    {
        return $this->getData('shipping_status') == self::SHIPPING_STATUS_COMPLETED;
    }
    ...
    {
        if ($this->isPending() || $this->isCanceled()) {
            throw new Exception('Magento Order creation is not allowed for Pending or Canceled Play.com Orders.');
        }
    }
