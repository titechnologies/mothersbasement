...
if (! $installer->getIsFirstInstall()) {
    ...
    if ($safeMode || !$canUse['file_exists'] || !$canUse['copy']) {
        ...
        $this->createNotice("Could not update TBT_Rewardssocial.csv",
        ...
        foreach ($locales as $locale) {
            ...
            if (!$isSafeToOverwrite) {
                // notify merchant of failure
                $this->createNotice("Could not backup TBT_Rewardssocial.csv ({$locale})",
                ...
                if (file_exists($source)) {
                    ...
                    if (!$isOverwriteSuccessful) {
                        // notify merchant of failure
                        $this->createNotice("Could not overwrite TBT_Rewardssocial.csv ({$locale})",
    ...
    if ($allOverwritesSuccessful) {
        $this->createNotice("TBT_Rewardssocial.csv has been replaced",
