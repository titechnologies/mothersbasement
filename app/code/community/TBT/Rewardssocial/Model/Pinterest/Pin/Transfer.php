...
class TBT_Rewardssocial_Model_Pinterest_Pin_Transfer extends TBT_Rewards_Model_Transfer
{
    /**
     ...
     * @return unknown
     */
    public function setPinterestPinId($id)
    {
        $this->clearReferences();
        $this->setReferenceType(TBT_Rewardssocial_Model_Pinterest_Pin_Reference::REFERENCE_TYPE_ID);
        $this->setReferenceId($id);
        $this->setReasonId(TBT_Rewards_Model_Transfer_Reason::REASON_CUSTOMER_DISTRIBUTION);
        $this->setData(TBT_Rewardssocial_Model_Pinterest_Pin_Reference::REFERENCE_KEY, $id);

        return $this;
    }

    public function isPinterestPin()
    {
        return $this->getReferenceType() == TBT_Rewardssocial_Model_Pinterest_Pin_Reference::REFERENCE_TYPE_ID;
    }
     ...
     */
    public function getTransfersAssociatedWithPinterestPin($pinterestPinId)
    {
        return $this->getCollection()
            ->addFilter('reference_type', TBT_Rewardssocial_Model_Pinterest_Pin_Reference::REFERENCE_TYPE_ID)
            ->addFilter('reference_id', $pinterestPinId);
    }
     ...
     * @return TBT_Rewards_Model_Special_Validator
     */
    protected function _getPinterestPinValidator()
    {
    ...
    public function create($customerId, $pinId, $rule)
    {
        ...
        if ($rule->getOnholdDuration() > 0) {
            ...
            $initial_status = Mage::getStoreConfig('rewards/InitialTransferStatus/AfterPinterestPin', $store);
