...
class TBT_Rewards_Block_Manage_Promo_Quote_Edit extends Mage_Adminhtml_Block_Widget_Form_Container {
	...
	public function __construct() {
        ...
        var rule_points_discount_action_option;
        ";
		...
		$this->_formInitScripts [] = "
	    	rule_points_discount_action_option = $$('select#rule_points_discount_action option[value=buy_x_get_y]')[0];
	        function toggleActionsSelect(action) {  
	    		...
	    		if(action == '$discount_by_points_spent') {
                    if($$('label[for=rule_points_discount_amount]').length == 1) { 
                    	$$('label[for=rule_points_discount_amount]')[0].innerHTML = '{$spent_discount_label}';
	    				$('rule_points_discount_amount').addClassName('validate-notzero');
	    			}
	    			if($$('select#rule_points_discount_action option[value=buy_x_get_y]').length == 1) {
	    			    $$('select#rule_points_discount_action option[value=buy_x_get_y]')[0].replace('');
	    			}
                } else {
                    if($$('label[for=rule_points_discount_amount]').length == 1) { 
                    	$$('label[for=rule_points_discount_amount]')[0].innerHTML = '{$old_discount_fixed_label}';
	    				$('rule_points_discount_amount').removeClassName('validate-notzero');
	    			}
	    			if($$('select#rule_points_discount_action option[value=buy_x_get_y]').length < 1) {
	    				if($$('select#rule_points_discount_action').length == 1) {
	    			    	$$('select#rule_points_discount_action')[0].insert(rule_points_discount_action_option);
	    			    }
	    			}
                }
	    	}
		...
		$this->_formInitScripts [] = "
	        function toggleDiscountActionsSelect(action) {
    	        var rule_points_discount_amount_row = $('rule_points_discount_amount').up().up();
    	        if ($('rule_points_discount_amount_container') != undefined){
    				rule_points_discount_amount_row = $('rule_points_discount_amount_container').up().up();
    			}
...
/*        	    var rule_discount_qty_row = $('rule_discount_qty').up().up();
	    		...
	    		if(action == '$no_discount') { 
    				rule_points_discount_amount_row.hide();
/*    				rule_discount_qty_row.hide();
    				rule_discount_step_row.hide();*/
    				rule_simple_free_shipping_row.hide();
	    			if($('rule_points_discount_amount').value == '') {
	    				$('rule_points_discount_amount').value = 0;
    				}
	    		} else {
    				rule_points_discount_amount_row.show();
/*    				rule_discount_qty_row.show();
		...
		if ($this->_getRule ()->isRedemptionRule ()) {
			$this->_formInitScripts [] = "toggleDiscountActionsSelect($('rule_points_discount_action').value)";
		}
