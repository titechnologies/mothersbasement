 ...
 */
class TBT_Rewards_Block_Manage_Promo_Quote_Edit_Tab_Main extends TBT_Rewards_Block_Admin_Widget_Form_Abstract {
	...
	protected function _prepareForm() {
		...
		if (Mage::helper ( 'rewards' )->isBaseMageVersionAtLeast ( '1.4.1.0' )) {
            ...
            // for now we do not support Auto-generation of Coupons Codes, introduced in 1.7 (ST-1831)
            if (isset($options[Mage_SalesRule_Model_Rule::COUPON_TYPE_AUTO])) {
                unset($options[Mage_SalesRule_Model_Rule::COUPON_TYPE_AUTO]);
            }
			$couponTypeFiled = $fieldset->addField ( 'coupon_type', 'select', array ('name' => 'coupon_type', 'label' => Mage::helper ( 'salesrule' )->__ ( 'Coupon' ), 'required' => true, 'options' => $options ) );
			
		    Mage::getSingleton('rewards/wikihints')->addWikiHint($couponTypeFiled, "21632117", "Shopping Cart Rule - Coupons" );
		    ...
		    Mage::getSingleton('rewards/wikihints')->addWikiHint($couponCodeFiled, "21632117", "Shopping Cart Rule - Coupons" );
		...
		Mage::getSingleton('rewards/wikihints')->addWikiHint($element, "21466162", "Rule Priority", null, $this->__("Get help with rule priorities."));
		...
		if (Mage::helper ( 'rewards' )->isBaseMageVersionAtLeast ( '1.4.1.0' )) {
			...
			Mage::dispatchEvent ( 'rewards_block_manage_promo_quote_edit_tab_main_prepare_form', array ('form' => $form ) );
		} else {
			$this->setForm ( $form );
		}
