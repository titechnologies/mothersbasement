...
/**
 * Used to render the Developer/DevMode field on the Config page using a non-default config path
 * @category   TBT
 * @package    TBT_Rewards
...
{
    /**
     * Renders the element into HTML, using the Developer Mode flag as the value to render
     *
    ...
    {
        $isDevMode = Mage::getStoreConfig('rewards/platform/dev_mode');
        $element->setValue($isDevMode);

        return parent::render($element);
    }

    /**
     * If account connected, disable drop-down select box, because we can't just change the Developer Mode and
    ...
    protected function _getElementHtml(Varien_Data_Form_Element_Abstract $element)
    {
        ...
        $html .=  Mage::helper('adminhtml')->__('<p class="note"><span><strong>Note:</strong> You need to disconnect your account before changing this.</span></p>');
