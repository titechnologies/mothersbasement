...
class TBT_Rewards_Block_Adminhtml_Sales_Order_View_Tab_Points extends Mage_Adminhtml_Block_Widget_Grid implements Mage_Adminhtml_Block_Widget_Tab_Interface {
	...
	protected function _prepareCollection() {
		...
		// TODO: remove this load and move the revokers stuff into _afterLoadCollection
		$this->collection->load();
		
		// include any transfers that have revoked the original transfers
		$revokers = $this->collection->selectRevokerTransfers();
		$revokers->selectPointsCaption('points');
		foreach ($revokers as $revoker) {
			$this->collection->addItem($revoker);
		}
