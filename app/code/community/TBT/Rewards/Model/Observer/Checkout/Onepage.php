...
class TBT_Rewards_Model_Observer_Checkout_Onepage extends Varien_Object {
    ...
    public function refreshCartBeforeCheckout($observer) {
        ...
        try{
            // Recalculate the quote object totals , to ensure that the quote object totals are not out dated.
            $this->_getQuote()->collectTotals();
            $this->_getQuote()->save();
        }catch(Exception $ex){
            Mage::helper('rewards')->log("Exception occured while recalculating the totals : {$ex->getMessage()}");
        }
