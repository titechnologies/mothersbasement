	...
	protected function _calcTaxAmounts(&$item) {
		...
		if ($item->getHasChildren() && $item->isChildrenCalculated()) {
			...
			foreach ($item->getChildren() as $child) {
				$childTax = $this->_calculator->calcTaxAmount ( $child->getRowTotal (), $child->getTaxPercent (), false, false );
				$childBaseTax = $this->_calculator->calcTaxAmount ( $child->getBaseRowTotal (), $child->getTaxPercent (), false, false );
        ...
        // no need to recalculate and set row_total_incltax and set base_row_total_incltax, this is already set
        // commented out to fix ST-1236
